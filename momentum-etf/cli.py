...
def show_current_portfolio(investment_amount=1000000, portfolio_size=5):
    ...
    try:
        ...
        for i, (ticker, score) in enumerate(top_etfs, 1):
            ...
            weight = (actual_investment / investment_amount) * 100 if investment_amount > 0 else 0
        ...
        print(tabulate(momentum_data, headers=["Rank", "ETF", "Score"], tablefmt="simple"))
...
def show_rebalancing_needs():
    ...
    print("=" * 70)
    print("💡 To use this feature, please edit the function to input your current holdings")
...
def show_historical_portfolio(from_date_str, to_date_str=None, investment_amount=1000000, portfolio_size=5):
    ...
    try:
        ...
        if from_date > to_date:
            print(f"❌ Error: From-date ({from_date_str}) cannot be after To-date ({to_date.strftime('%Y-%m-%d')})")
            return
    ...
    try:
        ...
        print(f"📡 Fetching historical data from {data_start.strftime('%Y-%m-%d')} to {to_date.strftime('%Y-%m-%d')}...")
        ...
        if eligible_tickers:
            ...
            from_momentum_scores = strategy.momentum_calculator.calculate_momentum_scores(eligible_from_data)
            from_top_etfs = [(ticker, score) for ticker, score in from_momentum_scores.items()][:portfolio_size]
            ...
            for i, (ticker, score) in enumerate(from_top_etfs, 1):
                ...
                from_table_data.append([
                    i, ticker, f"₹{price:.2f}", f"{units:,}", 
                    f"₹{actual_investment:,.0f}", f"{score:.4f}"
                ])
        ...
        if eligible_tickers:
            ...
            to_momentum_scores = strategy.momentum_calculator.calculate_momentum_scores(eligible_to_data)
            to_top_etfs = [(ticker, score) for ticker, score in to_momentum_scores.items()][:portfolio_size]
        ...
        if to_sell:
            ...
            for ticker in to_sell:
                from_units = int((investment_amount / portfolio_size) / from_prices[ticker])
        ...
        if from_portfolio_value > 0:
            ...
            if days_diff >= 30:
                annualized_return = ((to_portfolio_value / from_portfolio_value) ** (365 / days_diff) - 1) * 100
        ...
        if bought_etfs:
            ...
            if bought_data:
                headers = ["ETF", "Period Start Price", "Entry Price", "Period Performance"]
...
Examples:
  ...
  uv run cli.py backtest --amounts 1000000 5000000  # Test specific amounts
    ...
    portfolio_parser = subparsers.add_parser('portfolio', help='Show current optimal portfolio with allocations')
    ...
    rebalance_parser = subparsers.add_parser('rebalance', help='Show rebalancing needed for existing portfolio')
    
    # Historical command - Show portfolio between two dates
    historical_parser = subparsers.add_parser('historical', help='Show portfolio changes between two dates')
    ...
    try:
        if args.command == 'portfolio':
            ...
            run_multi_investment_backtest(args.amounts)
